...
def backup(path: Path):
    backup_dir = ROOT / "backups" / f"custom_sidebar_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def append_css_to_apply_design(text: str) -> str:
    """
    Добавляем CSS внутрь apply_design перед </style>.
    """
    ...
    if idx == -1:
        print("Could not find </style>, appending CSS as separate markdown block.")
        extra = f'''
st.markdown("""
<style>
{CSS_APPEND}
...
def main():
    ...
    print("=== Add custom desktop sidebar v2 ===")
